...
// MARK: - Welcome
struct TsnResponseModel: Codable {
    let id: Int
    let name, desc, shortDesc, type: String
    let owner: Owner
    let episode: JSONNull?
    let agvotCode: String
    let agvotDisclaimer: JSONNull?
    let qfrCode: String
    let airingOrder, broadcastDate, broadcastTime, broadcastDateTime: JSONNull?
    let lastModifiedDateTime: Date
    let gameID, album: JSONNull?
    let genres, keywords, tags: [JSONAny]
    let images: [Image]
    let authentication: Authentication
    let nextAuthentication: JSONNull?
    let ratingWarnings, people: [JSONAny]
    let funding: JSONNull?
    let musicLabels, broadcastNetworks: [JSONAny]
